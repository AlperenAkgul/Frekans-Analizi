...
Harflerin frekansina göre desifreleme yapan program

sifrelemek ve desifrelemek istediginiz metni normal.txt icerisine tamami kücük harf olacak sekilde atiniz.

Ingilizce harf frekans tablosu: https://commons.wikimedia.org/wiki/File:English_letter_frequency_%28frequency%29.svg
...
int main(){
    ...
    for(i = 0; i < 26; i++){
        for(j = i + 1; j < 26; j++){
            if(frekans[i] < frekans[j]){
                ...
                geciciChar = sifrelemeAlfabesi[i];
                sifrelemeAlfabesi[i] = sifrelemeAlfabesi[j];
                sifrelemeAlfabesi[j] = geciciChar;
            }
        }   
    }
    ...
    while(sifreliKarakter != EOF){
        for(i = 0; i < 26; i++){
            if(sifrelemeAlfabesi[i] == sifreliKarakter){
